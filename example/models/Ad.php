...
class Ad extends Model {
    ...
    public function editAd($id_ad, $title, $description, $price, $id_category, $state){
        ...
        //$sql = $pdo->query("UPDATE ads SET title = '$title', description = '$description', price = '$price', state = $state, id_category = '$id_category' WHERE id = ".$id_ad);
        $sql->bindValue(':title', $title);
        $sql->bindValue(':description', $description);
        $sql->bindValue(':price', $price);
        $sql->bindValue(':state', $state);
    ...
    public function countAds($filters = array('category' => '', 'price' => '', 'state' => '')){
        $arrayFilters = array('1=1');
